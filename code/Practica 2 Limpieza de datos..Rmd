...
test_normalidad = function(df, alpha = 0.05) {
  ...
  # Añadimos dichas variables a la lista de variables que no siguen una distribución normal
...
El análisis de la homogeneidad lo vamos a realizar con el fin de saber qué estadístico tenemos que aplicar para el contraste de hipótesis, por lo que vamos a tener cuenta las muestras de el ranking de un juego cuya categoría sea "GAME WORD" y con descargas altas, por otro lado el ranking de un juego cuya categoría sea "GAME WORD" y con descargas bajas:
...
#### ¿Qué variables cuantitativas influyen más en el ranking?
...
Otra forma de calcular la correlación de forma más detalla junto con la distribución de cada variable y cómo de importante es cada una de ellas es haciendo uso de una librería externa, las conclusiones que obtenemos son las mismas que en el caso anterior:
```{r}
...
#### ¿El ranking de los juegos de la categoría GAME WORD es superior en caso de tener unas instalaciones elevadas?

Continuando con las pruebas estadísticas, ahora vamos a hacer un contraste de hipótesis de dos muestras sobre la media. Una muestra va a contener todos los juegos de la categoría GAME WORD cuyas descargas sean inferiores a 10 millones, y la seguna muestra va a contener los juego de la categoría GAME WORD cuyas descargas sean iguales o superiores a 10 millones:
```{r}
